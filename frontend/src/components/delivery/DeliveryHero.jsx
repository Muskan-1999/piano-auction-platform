...
export default function DeliveryHero() {
  const { t } = useLanguage()
  return (
    <section className="relative flex items-center justify-center bg-cover bg-center bg-no-repeat"
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center text-center px-6 py-16" style={{ minHeight: 'clamp(350px, 40vw, 500px)' }}>
        <p className="text-[11px] uppercase tracking-[0.3em] text-white/60 mb-3">{t('delivery.pageLabel')}</p>
        ...
        <nav className="text-xs text-white/60 flex items-center justify-center gap-1 mt-2">
          <Link to="/" className="hover:text-white transition-colors">{t('delivery.breadcrumbHome')}</Link>
          <span className="mx-1">»</span>
          <span className="text-white/80">{t('delivery.breadcrumbPage')}</span>
        </nav>
      </div>
    </section>
  )
