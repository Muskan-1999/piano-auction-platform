...
const VALUATION_TYPES = [
  ...
  { value: 'Sale Valuation',      label: 'Sale Valuation' },
]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <SelectField
          label="Type of Valuation" required
      ...
      {/* Colour + Serial */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Piano Colour" required error={errors.piano_colour}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Dimensions" error={errors.dimensions}>
          <input type="text" name="dimensions" value={data.dimensions} onChange={handle}
            placeholder="Uprights – height in cm. Grands – overall length incl. lid" className={inputCls} />
        </Field>
        <Field label="Age of Piano" error={errors.age_of_piano}>
          <input type="text" name="age_of_piano" value={data.age_of_piano} onChange={handle}
            placeholder="e.g. 1985, approx. 30 years" className={inputCls} />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        />
        <SelectField
          label="Tuned Recently" required
          name="tuned_recently" value={data.tuned_recently}
          onChange={handle} options={YES_NO}
          placeholder="Select"
          error={errors.tuned_recently}
        />
        <SelectField
          label="Reconditioned" required
      ...
      {/* Ownership History */}
      <Field label="Ownership History" required error={errors.ownership_history}>
        <textarea
          name="ownership_history" value={data.ownership_history} onChange={handle} rows={3}
          placeholder="Please could you tell us briefly about the piano. Is it a family heirloom/recent purchase, how long have you owned it etc."
          className={`${inputCls} resize-none`}
        />
      </Field>

      {/* Condition */}
      <Field label="Condition Description" required error={errors.condition_description}>
        <textarea
          name="condition_description" value={data.condition_description} onChange={handle} rows={3}
          placeholder="Describe any scratches, damage, repairs or general condition"
          className={`${inputCls} resize-none`}
        />
      </Field>
      ...
      <Field label="Additional Notes" error={errors.additional_notes}>
        <textarea
          name="additional_notes" value={data.additional_notes} onChange={handle} rows={2}
      ...
      {/* Image Upload */}
      <div>
        <label className={labelCls}>Piano Image(s)</label>
        <p className="text-xs text-gray-500 mb-2">
          Maximum 5 images · jpg, jpeg, png, webp · 5MB each
        </p>
        <input
          ref={fileRef}
          type="file"
